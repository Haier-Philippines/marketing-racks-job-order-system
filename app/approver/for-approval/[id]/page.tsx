...
import { useAuthStore } from '@/stores'
import type { JobOrderRequest } from '@/types'
import { formatDate, formatDateTime, formatBytes, fetchImageAsDataUrl, cn } from '@/lib/utils'
import {
  ChevronLeft, CheckCircle, XCircle, Clock, Printer,
  Download, Paperclip, MessageSquare, User, ArrowRight, FileText,
} from 'lucide-react'
...
// Waits for all <img> tags in the print window to finish loading (or fail)
// before invoking print(), instead of relying on a fixed timeout which is
// unreliable for external images (e.g. Cloudinary attachments).
function printWhenImagesReady(w: Window) {
  const run = () => {
    const imgs = Array.from(w.document.images)
    if (imgs.length === 0) { w.focus(); w.print(); return }
    let settled = 0
    const done = () => { settled++; if (settled === imgs.length) { w.focus(); w.print() } }
    imgs.forEach(img => {
      if (img.complete) done()
      else { img.addEventListener('load', done); img.addEventListener('error', done) }
    })
    setTimeout(() => { if (settled < imgs.length) { w.focus(); w.print() } }, 4000)
  }
  setTimeout(run, 100)
}
    ...
    w.document.write(`
      ...
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .details-table, .approval-table { width: 100%; border-collapse: collapse; font-size: 11px; margin-top: 4px; }
        .details-table th, .approval-table th { background: #f3f4f6; text-align: left; padding: 6px 10px; border: 1px solid #e5e7eb; font-size: 10px; text-transform: uppercase; }
        .details-table td, .approval-table td { padding: 6px 10px; border: 1px solid #e5e7eb; }
...
if (req.attachments?.actualPhoto || req.attachments?.storePlan || req.attachments?.recommendation) {
  ...
  if (req.attachments?.actualPhoto) {
    ...
    doc.text('Actual Photo of Haier Space:', m, y); y += 5

    const imgData = await fetchImageAsDataUrl(req.attachments.actualPhoto.url)
    if (imgData) {
      const maxW = 90, maxH = 65
      ...
      doc.addImage(imgData.dataUrl, imgData.format, m, y, w, h)
      y += h + 4
      doc.setFontSize(8); doc.setTextColor(100,116,139)
      doc.text(req.attachments.actualPhoto.name, m, y)
      y += 8
    } else {
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleDownloadPDF} disabled={pdfLoading} className="btn-secondary btn-sm flex items-center gap-1.5">
              {pdfLoading ? <Spinner size={13} /> : <Download size={13} />} PDF
            </button>
            {canApprove && (
              <button onClick={() => setShowReview(true)}
                className="btn-primary flex items-center gap-2">
            ...
            <div className="section">
              ...
              {storeStatusLabel && (
                <div className="field-row">
                  ...
                  <span className="status-tag">{storeStatusLabel}</span>
                </div>
              )}
            </div>
            ...
            {req.requestDetails?.length > 0 && (
              <div className="section">
                ...
                <table className="details-table">
                  <thead>
                    <tr>
                      <th>#</th><th>Category</th><th>Qty</th><th>Type of Rack</th>
              ...
              <div className="section">
                ...
                {req.attachments?.storePlan && (
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#0066cc' }}>{req.attachments.storePlan}</div>
                  </div>
                )}
                {req.attachments?.recommendation && (
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 'bold', marginBottom: 6 }}>Recommendation</div>
                    <div style={{ fontSize: 10, color: '#374151' }}>{req.attachments.recommendation}</div>
                  </div>
                )}
              </div>
            )}
            ...
            {req.approvers.length > 0 && (
              <div className="section">
                ...
                <table className="approval-table">
                  <thead>
                    <tr>
                      <th>Approver</th><th>Role</th><th>Action</th><th>Date</th><th>Comments</th>
                    </tr>
                  </thead>
                  <tbody>
                    {req.approvers.map((a, i) => (
                      <tr key={i}>
                        ...
                        <td>{a.action}</td>
                        <td>{a.timestamp ? formatDate(a.timestamp) : '—'}</td>
                        <td>{a.comments ?? '—'}</td>
                      </tr>
                    ))}
                  </tbody>
