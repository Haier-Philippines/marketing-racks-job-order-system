...
import { useAuthStore } from '@/stores'
import type { JobOrderRequest } from '@/types'
import { formatDate, formatDateTime, formatBytes, fetchImageAsDataUrl } from '@/lib/utils'
import {
  ChevronLeft, Printer, Download, X, CheckCircle,
  Clock, AlertCircle, FileText, Phone, User, MapPin
} from 'lucide-react'
...
// before invoking print(). A fixed timeout is unreliable for external
// images (e.g. Cloudinary), which is why attachments sometimes appeared
// missing in the printed output.
function printWhenImagesReady(w: Window) {
  const run = () => {
    ...
    // Safety fallback in case something never fires
...
function RequestDetailContent() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Attachments — embed the actual photo, list other attachments as text
      if (req.attachments?.actualPhoto || req.attachments?.storePlan || req.attachments?.recommendation) {
        ...
        if (req.attachments?.actualPhoto) {
          ...
          doc.text('Actual Photo of Haier Space:', margin, y); y += 5
          ...
          if (imgData) {
            ...
            doc.addImage(imgData.dataUrl, imgData.format, margin, y, w, h)
            y += h + 4
            doc.setFontSize(8); doc.setTextColor(107,114,128)
            doc.text(req.attachments.actualPhoto.name, margin, y)
            y += 8
          } else {
            doc.setTextColor(0,0,0)
            doc.text(req.attachments.actualPhoto.name, margin, y)
            ...
            {canPrintOrDownload && (
              <>
                ...
                <button onClick={handleDownloadPDF} disabled={pdfLoading} className="btn-secondary btn-sm flex items-center gap-1.5">
                  {pdfLoading ? <Spinner size={13} /> : <Download size={13} />} Download PDF
                </button>
              </>
            )}
            {canCancel && (
              ...
              <div className="section">
                ...
                <div style={{ marginTop: 12 }}>
                  {req.attachments?.actualPhoto && (
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 11, fontWeight: 'bold', marginBottom: 6 }}>Actual Haier Space Photo</div>
                      <img src={req.attachments.actualPhoto.url} alt="Actual photo" crossOrigin="anonymous" style={{ width: '100%', maxWidth: '400px', border: '1px solid #e5e7eb', borderRadius: '6px' }} />
                      <div style={{ fontSize: 9, color: '#6b7280', marginTop: 3 }}>{req.attachments.actualPhoto.name}</div>
                    </div>
                  )}
                  {req.attachments?.storePlan && (
